...
public static class ImageBuilder
{
    ...
    static ImageBuilder()
    {
        ...
        try
        {
            ...
            if (File.Exists(fontPath))
            {
                ...
                _font = _fontFamily.CreateFont(36);
                Logs.Init($"Font loaded: {_fontFamily.Name}");
            }
            else
            {
    ...
    /// <summary>
    /// Builds a player image for a track.
    /// Downloads the album artwork, applies visual effects, and overlays track information.
    /// </summary>
    ...
    public static async Task<Image> BuildPlayerImageAsync(Dictionary<string, string> track)
    {
        try
        {
            ...
            Font font = _font ?? new Font(SystemFonts.Collection.Families.First(), 36);
        ...
        catch (Exception ex)
        {
            ...
            var fallbackImage = new Image<Rgba32>(800, 400, Color.DarkGray);
            return fallbackImage;
        }
    }

    /// <summary>
    /// Creates a rounded rectangle path for image clipping.
    /// Used to apply rounded corners to the player image.
    /// </summary>
