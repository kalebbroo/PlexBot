...
public class SelectMenus(ILogger<SelectMenus> logger, IAudioService audioService, PlexMusic plexApi, LavaLinkCommands lavaLink) : InteractionModuleBase<SocketInteractionContext>
{
    private readonly ILogger<SelectMenus> _logger = logger;
    ...
    public async Task DisplaySearchResults(string customId, string[] selections)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Error in DisplaySearchResults: {Message}", ex.Message);
        }
    }
